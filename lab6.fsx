...
let sine (degrees: float) : float = 
    Math.Sin(degrees * Math.PI / 180.0)

/// Косинус угла (в градусах)
let cosine (degrees: float) : float = 
...
// 2. ФУНКЦИИ ВЫСШЕГО ПОРЯДКА ДЛЯ ЛОГИРОВАНИЯ

/// Обёртка для логирования бинарных операций
let withLoggingBinary (operationName: string) (fn: float -> float -> float) (a: float) (b: float) : float =
...
// 3. ФУНКЦИИ ДЛЯ СОЗДАНИЯ ОПЕРАЦИЙ С ЛОГИРОВАНИЕМ

let createBinaryOperation name operation a b = 
    withLoggingBinary name operation a b

let createUnaryOperation name operation a = 
    withLoggingUnary name operation a

// 4. ВВОД ДАННЫХ ОТ ПОЛЬЗОВАТЕЛЯ

/// Чтение числа от пользователя с проверкой
...
// 7. ЗАПУСК ПРОГРАММЫ (для скрипта)
